...
func (k *KVStore) GetJobStatus(name, region string) (status *pb.JobStatus, err error) {
	...
	if res != nil {
		var es pb.JobStatus
		if err = jsonToPb(res.Value, &es); err != nil {
			return nil, err
		}
		Log.WithFields(logrus.Fields{
			"Name":         es.JobName,
...
func (k *KVStore) GetJobList(region string) (jobs []*pb.Job, err error) {
	...
	for _, entry := range res {
		var job pb.Job
		if err = jsonToPb(entry.Value, &job); err != nil {
			return
		}
...
func (k *KVStore) GetJob(name, region string) (*pb.Job, error) {
	...
	var job pb.Job
	if err = jsonToPb(res.Value, &job); err != nil {
		return nil, err
	}
	return &job, nil
}
...
func (k *KVStore) SetJob(job *pb.Job) error {
	...
	if err := k.Client.Put(jobkey, json, nil); err != nil {
		return err
	}
	return nil
}
